...
- **Two orthogonal axes of state coexist**: the *filter term* (what the user is typing) and the *selection set* (what the user has checked). The single most load-bearing invariant of this prompt is that **changing the filter must never drop selections**. Selection state is keyed by choice value/identity, not by visible-row index, so filtering only affects what is *rendered* - not what is *selected*. Any change that touches filtering, source refresh, or choice-list rebuilding must preserve this invariant. The `selection.test.ts` and `search-filtering.test.ts` suites guard it.
