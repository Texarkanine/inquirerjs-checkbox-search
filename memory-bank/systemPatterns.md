...
- **The prompt is built from `@inquirer/core` hooks** (`useState`, `useKeypress`, `useEffect`, `useMemo`, `usePagination`, `usePrefix`, `useRef`). It is not a class. State updates trigger re-render via the hook system; do not introduce module-level mutable state. As of `@inquirer/core` v12, `useState` setters treat every function argument as a reducer: when storing a choice `Value` that might itself be a function, always use reducer form (`setX(() => nextValue)`), never `setX(nextValue)`. Coverage: `object-references.test.ts` ("should store function-valued choices without invoking them as reducers").
...
Escape and backspace clear/edit the filter through `updateSearchTerm` (React state + readline rewrite). Syncing only from `rl.line` on keypress is insufficient under `@inquirer/testing`, whose `keypress('backspace')` emits a key event without mutating the readline line. Backspace deletes one **grapheme cluster**, segmented with the module-level `graphemeSegmenter` (`Intl.Segmenter`), so ZWJ sequences, flag pairs, skin-tone modifiers, and combining marks disappear in a single press. This is a deliberate divergence from Node readline's code-point deletion, decided in [#148](https://github.com/Texarkanine/inquirerjs-checkbox-search/issues/148): the prompt renders the search term itself, so a partial cluster is visible debris rather than an invisible internal detail. Any future search-term editing (delete key, word delete) should segment the same way. Coverage lives in `edge-cases.test.ts` ("backspace deletes one grapheme cluster").
