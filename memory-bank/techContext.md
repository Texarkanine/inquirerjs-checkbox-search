...
- Mutation testing (gated): StrykerJS via `stryker.config.json`; `npm run test:mutate:dry` / `npm run test:mutate`. PR workflow job _Mutation_ fails when the score drops below `thresholds.break` (80 — modest floor under the post-#147 cleaned score) or on harness crash / job timeout (see `.github/workflows/pr.yaml`). Justified `mutator.excludedMutations` / `// Stryker disable` entries are ledgered in `CONTRIBUTING.md`. Targeted range runs: `npx stryker run --mutate "src/index.ts:<start>-<end>" --reporters clear-text --incrementalFile /tmp/stryker-range.json` (use a fresh nonexistent `--incrementalFile`; config enables incremental by default, and `--force` against the shared report pollutes range scores).
