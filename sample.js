...
const fruits = [
  ...
  { name: 'Tomato', value: 'tomato', emoji: '🍅', description: 'Technically a fruit! 😉🍅' },
  ...
  { name: 'Peanut', value: 'peanut', emoji: '🥜', description: 'Technically a legume, but we\'ll allow it! 🥜' }
];
...
const selectedFruits = await checkboxSearch({
  ...
  choices: fruits.map(fruit => ({
    name: fruit.name,
    value: fruit.value,
    description: fruit.description || `Add delicious ${fruit.emoji} to your basket`
  })),
  ...
  instructions: 'Type to search • Tab to select • Enter to confirm • ↑↓ Navigate'
});
...
if (selectedFruits.length === 0) {
  ...
  selectedFruits.forEach((fruit, index) => {
    ...
    console.log(`   ${index + 1}. ${fruitData?.name || fruit} ${fruitData?.emoji || '🍎'}`);
  });
  ...
  if (selectedFruits.includes('tomato')) {
    console.log('🍅 The eternal question: Is a tomato a fruit or vegetable? Science says fruit! 🍅');
  }
